...
// Skidorterna kommer som prop från serverkomponenten i page.js. Tidigare
// hämtades de i en useEffect, vilket gjorde att sidans första HTML-svar
// innehöll en tom lista — sökmotorer och långsamma uppkopplingar såg
// noll orter. Nu ligger korten i markup:en direkt.
export default function HomeClient({ resorts }) {
  ...
  const countries = [
    ALLA_LANDER,
    ...[...new Set(resorts.map(r => r.country).filter(Boolean))]
      .sort((a, b) => land(a).localeCompare(land(b), 'sv')),
  ];
  ...
  const filtered = resorts.filter(r => {
    ...
    const matchText = !q
      ...
      || land(r.country)?.toLowerCase().includes(q)
      || r.notes?.toLowerCase().includes(q);
    return matchText && (country === ALLA_LANDER || r.country === country);
  });
