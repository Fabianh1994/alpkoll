...
function MagBtn({ children, href, primary = false }) {
  ...
  const style = {
    ...
    ...(primary ? {
      ...
      color: hover ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.6)',
...
function ResortCard({ resort, index }) {
  ...
  const flags = {
    ...
    'Italy': '🇮🇹', 'Andorra': '🇦🇩', 'New Zealand': '🇳🇿', 'Bulgaria': '🇧🇬',
  };
        ...
        >
          <div style={{ position: 'relative', height: 200, overflow: 'hidden', background: '#141210' }}>
            ...
            <div style={{
              ...
              padding: '4px 10px', borderRadius: 4, letterSpacing: '0.03em',
            }}>
              {flags[resort.country] || ''} {resort.country}
            </div>
          </div>
          <div style={{ padding: '16px 18px 24px' }}>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--font-body)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 4 }}>
              {resort.region}
            </div>
            <h3 style={{ fontSize: 24, fontFamily: 'var(--font-heading)', fontWeight: 400, color: '#f0ece4', margin: '0 0 10px', lineHeight: 1.1, letterSpacing: '0.03em' }}>
              {resort.name}
            </h3>
            {resort.notes && (
              <p style={{ fontSize: 13, fontFamily: 'var(--font-body)', color: 'rgba(255,255,255,0.4)', fontWeight: 300, margin: 0, lineHeight: 1.6 }}>
                {resort.notes}
              </p>
            )}
          </div>
    ...
    <div style={{ background: '#121110', minHeight: '100vh', color: '#f0ece4' }}>
      ...
      <header style={{
        ...
        <div style={{
          ...
          transform: `scale(1.06) translateY(-${parallaxY * 0.25}px)`,
        ...
        <div style={{ position: 'relative', maxWidth: 750 }}>
          <p style={{
            ...
            color: '#D4A574', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 20,
            opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(18px)',
            transition: 'all 0.9s cubic-bezier(0.16,1,0.3,1) 0.3s',
          }}>For skiers who do their homework</p>

          <h1 style={{
            fontFamily: 'var(--font-heading)', fontSize: 'clamp(52px, 8vw, 100px)',
            fontWeight: 400, lineHeight: 0.95, color: '#f0ece4', marginBottom: 24, letterSpacing: '0.02em',
            opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(26px)',
          ...
          <p style={{
            ...
            fontWeight: 300, color: 'rgba(255,255,255,0.5)', lineHeight: 1.65, maxWidth: 480, marginBottom: 40,
            opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(22px)',
      ...
      <section style={{ padding: '60px clamp(24px, 4vw, 64px)', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(40px,5.5vw,68px)', fontWeight: 400, color: '#f0ece4', lineHeight: 1, marginBottom: 22 }}>
            We Match The Mountain<br />To The <span style={{ color: '#D4A574' }}>Skier.</span>
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(13px,1.3vw,15px)', fontWeight: 300, color: 'rgba(255,255,255,0.38)', lineHeight: 1.75, maxWidth: 480, margin: '0 auto' }}>
            Snow depth, terrain mix, village vibe, budget, travel time — we score everything so you spend less time researching and more time actually skiing.
          </p>
        </div>
      </section>
      ...
      <section id="resorts" style={{ padding: '80px clamp(24px, 4vw, 64px) 120px', maxWidth: 1320, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 40, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            type="text" placeholder="Search by name, country or vibe..."
            value={search} onChange={e => setSearch(e.target.value)}
            style={{ flex: '1 1 260px', fontFamily: 'var(--font-body)', fontSize: 13, padding: '11px 16px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 4, color: '#f0ece4', outline: 'none' }}
          />
          <div style={{ display: 'flex', gap: 0, borderRadius: 4, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
            {countries.map(c => (
              <button key={c} onClick={() => setCountry(c)} style={{
                ...
                transition: 'all 0.2s', borderRight: '1px solid rgba(255,255,255,0.04)',
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
            {filtered.map((r, i) => <ResortCard key={r.slug} resort={r} index={i} />)}
          </div>
        )}
      </section>
