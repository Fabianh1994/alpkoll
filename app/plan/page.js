...
function scoreResort(resort, weights, skill) {
  ...
  const score = snowRaw * weights.snow + terrainRaw * weights.terrain + valueRaw * weights.value + accessRaw * weights.access + familyRaw * weights.family + apresRaw * weights.apres;
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
        {options.map(opt => {
          ...
          return (
            <button key={val} onClick={() => !isDisabled && onChange(val)} style={{
...
export default function PlanPage() {
  ...
  function runScoring() {
    ...
    const scored = resorts
      ...
      .map(r => ({ ...r, score: scoreResort(r, weights, skill), why: getWhyText(r, priorities) }))
                    ...
                    {showSuggestions && (
                      <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: T.card, border: '1px solid rgba(255,255,255,0.1)', borderTop: 'none', borderRadius: '0 0 6px 6px', maxHeight: 220, overflowY: 'auto' }}>
                        {AIRPORTS.filter(a => a.label.toLowerCase().includes(departureInput.toLowerCase())).map(a => (
                          <div key={a.code} onMouseDown={() => { setDepartureInput(a.label); setDepartureCode(a.code); setShowSuggestions(false); }} style={{ padding: '10px 16px', fontFamily: 'var(--font-body)', fontSize: 13, color: 'rgba(255,255,255,0.6)', cursor: 'pointer', borderBottom: `1px solid ${T.border}` }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(212,165,116,0.08)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>{a.label}</div>
                        ))}
                      </div>
                    )}
                  </div>
