...
export const DELOMRADEN = {
  salen: {
    ...
    forbindelse:
      'Paren hänger ihop med lift var för sig, men mellan dem går skidbussen, som är gratis med SkiPass.',
  ...
  chamonix: {
    ...
    forbindelse:
      'Områdena är inte förbundna med lift — bara Brévent och Flégère hänger ihop. Mellan dem går bussen i dalen.',
  },
}
...
/**
 * Kort mening om att talen är en summa, för ytor som inte har plats för
 * hela listan — jämförelsesidan och korten på ortsidan.
 */
export function summaMening(resort) {
  const delar = pistadeDelar(resort)
  if (!delar.length) return null

  // Andra meningen finns för höjdraderna längre ner i samma tabell. Utan
  // den kan läsaren räkna högsta minus lägsta och få ett tredje tal som
  // sajten inte visar någonstans.
  return `${resort.name} ${resort.total_pistes_km} km är ${antalOrd(delar.length)} områden som inte hänger ihop med lift. Störst är ${delar[0].namn} med ${delar[0].pist_km} km, och fallhöjden nedan är den största inom ett av dem.`
}
