...
import { harPris } from './liftkortspriser'
import { fallhojd } from './delomraden'
...
//
// Den gäller liftkortsfälten, som numera är de enda prisfälten.
...
// källan. Snöfallet står inte här — se kommentaren om avg_snowfall_cm
// längre ner.
//
// Här stod "Vad veckan kostar" som andra punkt, ur est_weekly_cost_eur.
// Den avgjorde alltså vilken ort som vann en jämförelse på ett tal som
// aldrig hämtats någonstans ifrån — se migration 024. Liftkortet, som är
// hämtat, står kvar i pristabellen längre ner.
...
export const HUVUDPUNKTER = [
  ...
  {
    // Ur höjderna, inte ur vertical_drop_m. Mätt 2026-08-30 går kolumnen
    // isär med topp minus botten för nitton av trettio orter, och åt båda
    // hållen — se fallhojd() i lib/ellerAlperna.js.
    etikett: 'Fallhöjd',
    riktning: 'hog',
    varde: (r) => fallhojd(r),
    visa: (r) => nummer(fallhojd(r)) && `${nummer(fallhojd(r))} m`,
...
export const GRUPPER = [
  ...
  },
  {
    ...
    falt: [
      ...
      // vertical_drop_m, som går isär med dem för nitton av trettio orter —
      // Ruka har 292 m fallhöjd på ett berg som spänner 201. Sälen och
      // Chamonix är undantagen: där korsar höjdspannet två skilda områden,
      // och fallhojd() i lib/delomraden.js tar den största inom ett av dem.
      { etikett: 'Fallhöjd', riktning: 'hog', varde: (r) => fallhojd(r), visa: (r) => nummer(fallhojd(r)) && `${nummer(fallhojd(r))} m` },
    ],
  },
  {
    rubrik: 'Terräng',
  ...
  },
  {
    rubrik: 'Pris',
    falt: [
      // harPris-spärren gäller här av två skäl, inte ett. Det uppenbara är
      // att ett obelagt tal inte ska visas. Det andra är att `varde` avgör
      // vilken ort som markeras som billigast — utan spärren hade Ruka
      // vunnit prisraden mot Åre på ett belopp som aldrig hämtats.
      { etikett: 'Dagskort',  riktning: 'lag', varde: (r) => (harPris(r) ? r.lift_pass_day_eur : null),  visa: (r, k) => (harPris(r) ? pris(r.lift_pass_day_eur, VALUTA(r), k) : null) },
      { etikett: 'Veckokort', riktning: 'lag', varde: (r) => (harPris(r) ? r.lift_pass_week_eur : null), visa: (r, k) => (harPris(r) ? pris(r.lift_pass_week_eur, VALUTA(r), k) : null) },
    ],
  },
]
...
export function jamforelseMeningar(a, b, kurser) {
  ...
  const fall = ordna(a, b, (r) => fallhojd(r))
  ...
  // Samma spärr som i tabellen ovan: en prismening får inte byggas på ett
  // tal vi inte kan belägga. Utan den påstod sidan att Ruka är billigare
  // än Åre med så och så många procent, på ett belopp ur databasen som
  // ingen prislista stöder.
  const vecka = ordna(a, b, (r) => (harPris(r) ? r.lift_pass_week_eur : null))
  ...
  // Här stod en mening till: "Hela veckan kostar uppskattningsvis X per
  // person i A mot Y i B, resa och boende inräknat." Den påstod en
  // kostnadsjämförelse ur est_weekly_cost_eur, ett fält som aldrig hämtats
  // någonstans ifrån — och tröskeln på 1,2 gick att passera enbart för att
  // de gissade talen låg ett femtiosteg isär. Se migration 024.
