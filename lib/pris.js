// Priser.
//
// Sajten visade tidigare "vad en vecka kostar" per ort, först som
// veckokortet plus ett påslag på 400–900 euro, sedan ur kolumnen
// est_weekly_cost_eur. Båda är borta sedan 2026-09-08, och fältet ska
// inte tas tillbaka. Underlaget för det står i migration 024: samtliga
// trettio värden var delbara med femtio, trettio orter delade på sexton
// tal, talet följde inte liftkortspriset, och fyra orter bar en
// veckokostnad utan att ha något känt liftkortspris alls. Kolumnen sattes
// aldrig i någon migration — den kom med den ursprungliga databasen och
// hade aldrig genomgått källkontroll.
//
// Det som återstår är liftkortet, som är hämtat ur ortens egen prislista.
// Vad resa och boende kostar beror på vecka, boendestandard och färdsätt,
// och är därför ingen egenskap hos orten alls.
//
 ...
 */
/**
 * Beloppet i kronor som tal — oavrundat och oformaterat.
 *
 * Finns för att /liftkortspriser sorterar trettio orter på pris och räknar
 * ut vad en skiddag kostar. Att sortera på den formaterade strängen hade
 * gett bokstavsordning, alltså "1 200 kr" före "950 kr".
 */
export function kronorTal(belopp, valuta, kurser) {
  if (!Number.isFinite(belopp) || belopp <= 0 || !kurser) return null

  const kod = String(valuta || 'EUR').toUpperCase()
  if (kod === 'SEK') return belopp
  ...
  return (belopp / perEuro) * kurser.SEK
}

/**
 * Vad en skiddag kostar med sexdagarskortet, i kronor avrundat till tiotal.
 *
 * Uträkningen stod inline på /liftkortspriser. Den ligger här sedan
 * ortsidans vanliga frågor skriver ut samma tal, så att en ort inte kan få
 * två olika dagspriser på samma sajt.
 */
export function perSkiddag(kronorForSexDagar) {
  if (!Number.isFinite(kronorForSexDagar) || kronorForSexDagar <= 0) return null
  return Math.round(kronorForSexDagar / 6 / 10) * 10
}

export function kronor(belopp, valuta, kurser) {
  const tal = kronorTal(belopp, valuta, kurser)
  return tal === null ? null : formateraKronor(tal)
}
