...
/**
 * Bilresan som en mening, till de sidor som förut hade den i transport_info.
 *
 * Ortstexterna skrevs om i migration 029 utan handskrivna restider, efter
 * docs/copy.md: tal skrivs inte för hand i löptext. Jämförelsesidorna och
 * alpsidorna visade transport_info som enda uppgift om resan, så där står
 * bilresan nu i stället, ur samma tal som ortsidans vanliga frågor.
 *
 * "Med bil är det 5,9 timmar från Stockholm, 6,9 från Göteborg och 10 från
 * Malmö, utan raster."
 *
 * @param stadNycklar  vilka städer som ska med. Till alporterna bara Malmö,
 *                     av samma skäl som i lib/vanligaFragor.js.
 */
export function bilMening(slug, stadNycklar = STADER.map((s) => s.nyckel)) {
  const resor = STADER
    .filter((s) => stadNycklar.includes(s.nyckel))
    .map((s) => ({ stad: s.namn, resa: bilresa(slug, s.nyckel) }))
    .filter((x) => x.resa)
  if (resor.length === 0) return null

  const tim = (minuter) => (minuter / 60).toFixed(1).replace('.', ',').replace(/,0$/, '')
  const [forsta, ...ovriga] = resor
  const delar = [`${tim(forsta.resa.minuter)} timmar från ${forsta.stad}`, ...ovriga.map((x) => `${tim(x.resa.minuter)} från ${x.stad}`)]
  const lista = delar.length > 1 ? `${delar.slice(0, -1).join(', ')} och ${delar.at(-1)}` : delar[0]
  return `Med bil är det ${lista}, utan raster.`
}
