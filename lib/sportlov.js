...
export function soldenLage(nr) {
  ...
  // Skidveckan börjar lördagen efter fredagsavgången och slutar den lördag
  // hemtåget går.
  const lordag = new Date(`${resa.ut}T12:00:00Z`)
  lordag.setUTCDate(lordag.getUTCDate() + 1)
  const start = lordag.toISOString().slice(0, 10)

  if (resa.hemFranAlperna <= SOLDEN_HOGPRIS_SLUT) return 'fore'
  if (start > SOLDEN_HOGPRIS_SLUT) return 'efter'
  return 'over'
}
