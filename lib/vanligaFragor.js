...
import { STADER, bilresa, linjeFor } from './restider'
...
function fragaFallhojd(ort, alla) {
  ...
  const delar = [`${meter(fh)}, från ${talet(ort.altitude_base)} till ${meter(ort.altitude_top)} över havet.`]
...
function fragaResa(ort, idag) {
  ...
  if (arNordisk(ort) && bil.length) {
    ...
    const linje = linjeFor(ort.slug, idag)
    if (linje) {
      ...
      delar.push(`${linje.rubrik} går ${period}.`)
      if (stockholm) delar.push(`Från Stockholm ${stockholm.avgang}, framme ${linje.framme}.`)
      if (linje.forbehall) delar.push(linje.forbehall)
    }
